...
public enum ChatPromptBuilder {
    ...
    public static func systemPrompt(vaultName: String, noteCount: Int) -> String {
        ...
        - Use `create_note` to save a new item (todo, book, movie, restaurant, note, etc.) — \
          ...
          singular type names (e.g. "book", not "books"), and avoid formatting variants with \
          symbols or punctuation (e.g. "to-do", "book!"). Only introduce a new type when the user \
